...
class WindowBert2Attention(BertSelfAttention):
    ...
    def forward(
        ...
        for i in range(query_layer.shape[2]):
            ...
            l, r = max(0, i-WINDOW_SIZE+1), min(query_layer.shape[2] - 1, i+WINDOW_SIZE)
            ...
            attn_weights_i_new = torch.zeros(attn_weights_i.shape[0], attn_weights_i.shape[1], 1, 2*WINDOW_SIZE+1).to(attn_weights_i.device)
